...
namespace DemonEngine {
    ...
    int Engine::gameLoop(float deltaTime) {
        ...
        if (deltaTime <= 0.0f) {
            ...
            for (uint s = 0; s < 20; s++) {
                _debugShader->uniformInt(fmt::format("_shadowMap[{}]", s), 4 + s);
                _debugShader->uniformInt(fmt::format("_shadowSpotMaps[{}]", s), 24 + s);
            }
            int lightNum = 0;
            for (auto light: _lightEntities) {
                if (light->getShadowHeight() > 0) {
                    ...
                    if (light->getCutoff() > 0.0f) {
                        ...
                        _debugShader->uniformInt(fmt::format("_shadowMap[{}]", lightNum), 2 + lightNum);
                    }
                }
                lightNum++;
            }
